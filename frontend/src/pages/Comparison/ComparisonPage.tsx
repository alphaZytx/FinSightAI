  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-surface-400">Select filings from two or more companies to benchmark shared financial evidence</p>
      </div>
      ...
      <Card title="Select Reports" subtitle="Choose one filing per company for the cleanest peer benchmark">
        <form onSubmit={submit} className="space-y-4">
          ...
          {analysis.isLoading && <p className="text-sm text-surface-400">Loading available filings…</p>}
              ...
              {documents.map((doc) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <Building2 className={`h-5 w-5 shrink-0 ${checked ? 'text-primary-400' : 'text-surface-500'}`} />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-surface-100 truncate">{doc.company_name}</p>
                      <p className="text-xs text-surface-400 truncate">{doc.file_name || doc.document_type} · FY {doc.fiscal_year || 'not stated'}</p>
                    </div>
          ...
          {!analysis.isLoading && documents.length === 0 && (
            <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-surface-200">No indexed filings available</p>
                <p className="text-xs text-surface-400">Upload company filings first, then return here to compare.</p>
              </div>
          ...
          {!ready && documents.length > 0 && (
            <div className="flex items-center gap-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
              <AlertCircle className="h-5 w-5 text-amber-400 shrink-0" />
              <div>
                <p className="text-sm font-medium text-surface-200">Another company filing needed</p>
                <p className="text-xs text-surface-400">Upload a similar report for a second company, then select both.</p>
              </div>
              <Link to="/workspace" className="ml-auto inline-flex items-center gap-1.5 rounded-lg bg-primary-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-primary-500">
                <FilePlus2 className="h-3.5 w-3.5" /> Upload
              </Link>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button
              type="submit"
              disabled={busy || !ready}
              className="inline-flex items-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white shadow-lg shadow-primary-600/20 hover:bg-primary-500 transition-colors disabled:cursor-not-allowed disabled:opacity-50"
            >
      ...
      {result && !result.eligibility.ready && (
        <Card>
          <div className="flex items-center gap-3">
            <AlertCircle className="h-5 w-5 text-amber-400 shrink-0" />
            <div>
              <p className="text-sm font-medium text-surface-200">Comparison needs more data</p>
              <p className="text-xs text-surface-400">{result.eligibility.message}</p>
            </div>
          </div>
        </Card>
      )}
            ...
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-surface-400">Companies compared</p>
                  <p className="text-xs text-surface-500">{result.coverage.selected_document_count} selected filings</p>
                </div>
              </div>
            </Card>
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-surface-400">Benchmark observations</p>
                  <p className="text-xs text-surface-500">{result.coverage.comparable_metric_rows} shared metric rows</p>
                </div>
              </div>
            </Card>
            <Card>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-surface-400">Risk signals</p>
                  <p className="text-xs text-surface-500">Across selected companies</p>
                </div>
              </div>
            </Card>
          </div>
            ...
            <Card title="Benchmark Observations" subtitle="Peer view across selected companies">
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {result.benchmark_insights.map((insight) => (
                  <div key={`${insight.metric_key}-${insight.period}-${insight.leader}`} className="rounded-lg border border-surface-700/40 bg-surface-900/40 p-4">
                    <Badge variant="info">{insight.period || 'Reported period'}</Badge>
                    <h4 className="mt-2 text-sm font-medium text-surface-100">{insight.title}</h4>
                    <p className="mt-1 text-xs text-surface-400">{insight.detail}</p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {result.comparison.map((row) => (
                      <tr key={`${row.metric_key}-${row.period ?? 'na'}`} className="border-b border-surface-700/20 last:border-0">
                        <td className="py-3 pr-4">
                          <p className="text-sm font-medium text-surface-100">{row.metric_name}</p>
                          <p className="text-xs text-surface-500">{row.normalization_note}</p>
                        </td>
                        <td className="py-3 pr-4 text-surface-300">{row.period ?? 'Not stated'}</td>
                        {companies.map((company) => {
                          ...
                          return (
                            <td key={company} className="py-3 pr-4">
                              {cell ? (
                                <div>
                                  <p className="font-semibold text-surface-100">{cell.display_value}</p>
                                  <p className="text-xs text-surface-500">p. {cell.source_page ?? 'n/a'} · {Math.round(cell.confidence * 100)}%</p>
                                </div>
                              ) : (
                                <span className="text-xs text-surface-500">Not reported</span>
                              )}
                            </td>
                          );
                        })}
            ...
            <Card title="Company Risk Summary" subtitle="Risk profile for each company">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {result.risk_summary.map((risk) => (
                  <div key={risk.company_name} className="rounded-lg border border-surface-700/40 bg-surface-900/40 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-sm font-medium text-surface-100">{risk.company_name}</h4>
                    ...
                    <p className="text-xs text-surface-500">Risk intensity score</p>
                    ...
                    {risk.examples.length > 0 && (
                      <p className="mt-1 text-xs text-surface-500 line-clamp-2">{risk.examples.join(' · ')}</p>
                    )}
                  </div>
                ))}
              </div>
