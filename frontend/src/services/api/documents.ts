...
export async function listDocuments(workspaceId: string): Promise<DocumentUploadResponse[]> {
  const { apiFetch } = await import('./client');
  return apiFetch<DocumentUploadResponse[]>(`/documents?workspace_id=${encodeURIComponent(workspaceId)}`);
}

export async function deleteDocument(documentId: string): Promise<void> {
  const { apiFetch } = await import('./client');
  await apiFetch<void>(`/documents/${encodeURIComponent(documentId)}`, { method: 'DELETE' });
}
